...
type IOrders interface {
	QueryOrders(userId uint64, orderType string, pageIndex uint) (*[]OrderForm, error)
}

// TODO uncompleted
func (of *OrderForm) QueryOrders(userId uint64, orderType string, pageIndex uint) (*[]OrderForm, error) {
	mysqlDB := db.GetMySqlConnection().GetMySqlDB()

	_orderType := strings.ToUpper(orderType)
	var orders []OrderForm
	if _orderType == "ALL" {
		mysqlDB.Where("userId = ?", userId).Order("updatedAt DESC").Find(&orders)
	} else {
		mysqlDB.Where("userId = ? status = ?", userId, _orderType).Order("updatedAt DESC").Find(&orders)
	}
	return &orders, nil
}
